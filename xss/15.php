            ...
            <header class="page-header">
                <div class="container-fluid">
                    <h2 class="no-margin-bottom">
						...
						-
						XSS Attack 15 - Reflected eval Base64 
					</h2>
                </div>
            </header>
            <section>
                <div class="container-fluid fa-lang">
					<form method="get" action="" autocomplete="off">
					  ...
					  <button type="submit" name="btn_send" value="1" class="btn btn-success">Show</button>
					</form>			
					  <br>
					  <br>
					  <div id="result">
...
<script>
	<?php
		if($main->get('btn_send'))
		{
			...
			if(stristr($a,'prompt'))
				die('waf !');
			if(stristr($a,'confirm'))
				die('waf !');	
			if(stristr($b,'alert'))
				die('waf !');
			if(stristr($b,'prompt'))
				die('waf !');
			if(stristr($b,'confirm'))
				die('waf !');			
			...
			$b = preg_replace('/<\/(.*?)s(.*?)c(.*?)r(.*?)i(.*?)p(.*?)t(.*?)>/i',"", $b);
			?>
				eval("c = <?php print $a; ?> + <?php print $b; ?>");
				$('#result').html(  'Concat is : <b>' +  c + '</b>');
			<?php
		}
	?>
</script>
