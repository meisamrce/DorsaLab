<?php	
	...
	if($main->get('logout'))
	{
		...
		$main->redirect(BASE_URL.'csrf/05.php?msg=ok');
	}		
?>
<!DOCTYPE html>
<html>
						...
						if(isset($_SESSION['admin_id']))
						{
							if($main->post('btn_save'))
							{
								if($_SESSION['csrf_token'] == $main->post('csrf_token'))
								{
									...
									$main->redirect(BASE_URL.'csrf/05.php?msg=err');
							}							
							print 'Login :)';
							generateToken();
							?>
							...
							<br>
							<a href="csrf/05-xss.php">XSS Test</a>
							<?php
						}
						else
						{
							if($main->post('btn_login'))
							{
								...
								if(isset($row['id']))
								{
									...
									$main->redirect(BASE_URL.'csrf/05.php');
								}
								else
								{
									print 'Login Error';
