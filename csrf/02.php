<?php	
	...
	if($main->get('logout'))
	{
		...
		$main->redirect(BASE_URL.'csrf/02.php?msg=ok');
	}	
					...
					<?php
						if(isset($_SESSION['admin_id']))
						{
							...
							<form method="post" action="" autocomplete="off">
							  ...
							  <button type="button" onclick="redirect('<?php print BASE_URL; ?>csrf/02.php?logout=1')" class="btn btn-danger">Logout</button>
							</form>								
							<?php
						}
						else
						{
							if($main->post('btn_login'))
							{
								...
								if(isset($row['id']))
								{
									...
									$main->redirect(BASE_URL.'csrf/02.php');
								}
								else
								{
									print 'Login Error';
